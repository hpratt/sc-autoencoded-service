...
# Create annotated-datasets-db-credentials secret if it does not exist
if ! kubectl get secret annotated-datasets-db-credentials &>/dev/null ; then
    echo "Creating annotated-datasets-db-credentials secret..."
    DB_USERNAME=$(jq -r ".annotated_datasets_db_username" /tmp/devops/staging-db-credentials/annotated-datasets-db.json)
    DB_PASSWORD=$(jq -r ".annotated_datasets_db_password" /tmp/devops/staging-db-credentials/annotated-datasets-db.json)
    kubectl create secret generic annotated-datasets-db-credentials \
        --from-literal=username=$DB_USERNAME \
        --from-literal=password=$DB_PASSWORD
else
    echo "Secret annotated-datasets-db-credentials already found. Skipping creation..."
fi
